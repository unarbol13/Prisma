...
class PhotoConfigScreen extends StatefulWidget {
  const PhotoConfigScreen({super.key});

  @override
  State<PhotoConfigScreen> createState() => _PhotoConfigScreenState();
}

class _PhotoConfigScreenState extends State<PhotoConfigScreen> {
  String timer = '3';
  String flash = 'auto';
  String filter = 'none';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('CONFIG FOTOGRAFÍA'),
      ),
      body: Container(
        color: Colors.black,
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              _picker('Temporizador (segundos)', timer, ['0', '3', '5', '10'], (v) => setState(() => timer = v)),
              _picker('Flash', flash, ['Auto', 'On', 'Off'], (v) => setState(() => flash = v.toLowerCase())),
              _picker('Filtro', filter, ['Ninguno', 'Blanco y Negro', 'Sepia', 'Vintage'], (v) => setState(() => filter = v == 'Ninguno' ? 'none' : v.toLowerCase().replaceAll(' ', '_'))),
